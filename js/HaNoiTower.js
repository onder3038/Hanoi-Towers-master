...
class game {
  ...
  listenMouse() {
    document.addEventListener("mousedown", (evt) => {
      ...
      if (x < 1.5 * this.getWidth() && y < this.getWidth()) this.Auto();
  ...
  move(start, end) {
    ...
    if (
      A[start][A[start].length - 1] > A[end][A[end].length - 1] ||
      start == end
    ) {
      this.rec[A[start][A[start].length - 1]].y =
        game_H - this.getWidth() / 2 - A[start].length * this.getWidth();
      this.rec[A[start][A[start].length - 1]].x = x[start];
      return;
    }

    this.rec[A[start][A[start].length - 1]].y =
      game_H - this.getWidth() / 2 - (A[end].length + 1) * this.getWidth();
  ...
  update() {
    if (auto && count % 30 == 0) {
      if (index < B.length) this.move(B[index].s, B[index].e);
      index++;
    }
    count++;
    if (messageWin && win && count++ > 0) {
      ...
      window.alert(
        "Sen kazandin CETO'nun gozleri yasli !\n" +
          "N = " +
          N +
          "\nHamle: " +
          Tur +
  ...
  render() {
    if (
      ...
      this.rec = [];
      for (let i = 0; i < N; i++)
        this.rec[N - i] = new rectangle(
          this,
          x[1],
          game_H - this.getWidth() / 2 - this.getWidth() * (i + 1),
          game_W / 3 - i * ((game_W / 3 - 1.5 * this.getWidth()) / (N - 1)),
          cl[i]
        );
    }
  }
  ...
  draw() {
    this.clearScreen();
    for (let i = 1; i <= N; i++) this.rec[i].draw();
    this.drawIcon();
  }
  ...
  otherNumber(a, b) {
    for (let i = 1; i <= 3; i++) if (a != i && b != i) return i;
    return -1;
  }

  getCol(x) {
    if (x < game_W / 3) return 1;
    if (x < (2 * game_W) / 3) return 2;
    return 3;
  }
